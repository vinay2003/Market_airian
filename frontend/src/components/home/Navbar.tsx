        ...
        >
            <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
                <Link to="/" className="flex items-center gap-2 group">
                    ...
                    <span className={`font-heading font-bold text-2xl tracking-tight transition-colors ${scrolled ? 'text-gray-900' : 'text-gray-900'}`}>MarketFly</span>
                </Link>

                {/* Desktop Nav */}
                <div className="hidden md:flex items-center gap-8">
                    {['Venues', 'Services', 'Gallery', 'About'].map((item) => (
                        <a key={item} href={`#${item.toLowerCase()}`} className="text-sm font-medium text-gray-600 hover:text-primary transition-colors">
                            {item}
                        </a>
                    ))}
                </div>
                ...
                <div className="hidden md:flex items-center gap-4">
                    <Link to="/login">
                        <Button variant="ghost" className="hover:bg-primary/5 hover:text-primary font-medium">Log in</Button>
                    </Link>
                    <Link to="/vendor/onboarding">
                        <Button className="bg-primary hover:bg-primary/90 text-white shadow-lg shadow-primary/25 rounded-full px-6 transition-all hover:scale-105 active:scale-95 duration-200">
                            List Your Business
                        </Button>
                    </Link>
                </div>
                ...
                >
                    ...
                    <div className="pt-4 flex flex-col gap-3">
                        ...
                        <Link to="/vendor/onboarding" onClick={() => setMobileMenuOpen(false)}>
                            <Button className="w-full justify-center bg-primary text-white">List Your Business</Button>
                        </Link>
                    </div>
                </motion.div>
            )}
